...
object MaskJson {
  ...
  def apply(json: Json): Json = applyWithCustomMask(json)(Json.fromString(StringMask), None)
  

  def applyWithCustomMask(json: Json)(customMask: Json = Json.fromString(StringMask), numberMasking: Option[Json] = None): Json = 
    if(json.isArray) 
  ...
  private def maskVectorJson(vector: Vector[Json])(customMask: Json, numberMasking: Option[Json]): Vector[Json] = {
    @tailrec
    def vectorLoop(unmaskedVector: Vector[Json], maskedVector: Vector[Json]): Vector[Json] = {
      if(unmaskedVector.isEmpty)
        maskedVector
      else
        vectorLoop(unmaskedVector.tail, maskedVector.appended(applyWithCustomMask(unmaskedVector.head)(customMask, numberMasking)))
    }
    vectorLoop(vector, Vector.empty)
  }

  private def maskJsonObject(jobj: JsonObject)(customMask: Json, numberMasking: Option[Json]): JsonObject = {
    @tailrec
    def objectLoop(unmaskedObject: Vector[(String, Json)], maskedObject: Vector[(String, Json)]): Vector[(String, Json)] = {
       if(unmaskedObject.isEmpty)
         maskedObject
          ...
          val maskedValue = applyWithCustomMask(valueToMask)(customMask, numberMasking)
